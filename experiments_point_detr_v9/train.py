"""Training script for Unsupervised Point-RT V9.
...
def main():
    ...
    for epoch in range(config.training.num_epochs):
        ...
        log_dict = {
            ...
            'train/render_loss': train_metrics['render_loss'],
            'val/loss': val_metrics['total_loss'],
            'val/sinkhorn_loss': val_metrics['sinkhorn_loss'],
            'val/repulsion_loss': val_metrics['repulsion_loss'],
            'val/render_loss': val_metrics['render_loss'],
        }
        
        print(f"\nEpoch {epoch}")
        print(
            f"  Train Loss: {train_metrics['total_loss']:.4f} "
            f"(Sinkhorn: {train_metrics['sinkhorn_loss']:.4f}, "
            f"Repulsion: {train_metrics['repulsion_loss']:.4f}, "
            f"Render: {train_metrics['render_loss']:.4f})"
        )
