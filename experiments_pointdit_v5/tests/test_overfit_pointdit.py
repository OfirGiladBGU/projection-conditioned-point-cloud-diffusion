...
def main():
    ...
    # Loss configuration
    parser.add_argument("--grid-density-weight", type=float, default=1.0, help="Weight for GridDensityLoss")
    parser.add_argument("--repulsion-weight", type=float, default=0.3, help="Weight for RepulsionLoss")
    parser.add_argument("--adaptive-repulsion", action="store_true", default=True, help="Use adaptive repulsion (default)")
    parser.add_argument("--no-adaptive-repulsion", dest="adaptive_repulsion", action="store_false", help="Use fixed-radius repulsion")
    ...
    print(f"      Point-DiT V5 model created")
    print(f"      Model parameters: {model.get_num_params():,}")

    print("[4/6] Creating scheduler...")
    ...
    print(f"[6/6] Loading sample {args.sample_index}...")
    ...
    print(f"Loss config: grid_density_weight={args.grid_density_weight}, repulsion_weight={args.repulsion_weight}, adaptive_repulsion={args.adaptive_repulsion}")
    print(f"{'Step':>5} {'Loss':>12} {'Chamfer':>12} {'Density':>12} {'Repulsion':>12}")
    print("-" * 60)
    ...
    for step in range(args.steps):
        ...
        if step % 25 == 0 or step == args.steps - 1:
            print(f"{step:5d} {loss.item():12.6f} {loss_dict['chamfer']:12.6f} {loss_dict['grid_density']:12.6f} {loss_dict['repulsion']:12.6f}")
