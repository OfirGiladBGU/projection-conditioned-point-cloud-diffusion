"""Overfit Point-DiT V6 on a single (source, target) pair.
...
V6 differences from V5:
  - Larger model (dim=256, 6 layers, 8 heads, ~5M params)
  - Tactile density input (use_density_input=True)
  - GPU Sinkhorn OT matching in train_step

Usage (from experiments_pointdit_v6/):
...
def main():
    ...
    # V6 OT matching options
    parser.add_argument("--ot-matching", action="store_true", default=True,
                        help="Enable GPU Sinkhorn OT matching (default: on)")
    parser.add_argument("--no-ot-matching", dest="ot_matching", action="store_false")
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            name=f"v6-overfit-idx{args.sample_index}-{args.steps}steps",
        )
    ...
    # ── model ─────────────────────────────────────────────────────────
    use_density = getattr(config.model, "use_density_input", True)
    model = PointDiT(
        n_points=N_POINTS,
    ...
    print(f"  PointDiT V6 trainable params: {trainable:,}")
    print(f"  use_density_input: {use_density}")
    print(f"  OT matching: {args.ot_matching}")
    ...
    for step in range(1, args.steps + 1):
        ...
        loss_dict = train_step(
            ...
            use_ot_matching=args.ot_matching,
            use_gpu_sinkhorn=True,
        )
        loss = loss_dict["loss"]
    ...
    if HAS_MPL:
        ...
        ax.set_xlabel("Step")
        ax.set_ylabel("Loss")
        ax.set_title(f"PointDiT V6 Overfit Loss ({fname})")
